...
const MicIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M7 4a3 3 0 016 0v4a3 3 0 11-6 0V4zm5 10.5a7.5 7.5 0 00-1.243-4.123A5.965 5.965 0 0112 8V4a4 4 0 10-8 0v4c0 .713.109 1.404.31 2.062A7.47 7.47 0 005 14.5V16h1.5v-1.5a.5.5 0 01.5-.5h6a.5.5 0 01.5.5V16H15v-1.5z" clipRule="evenodd" /></svg>;
...
export default function App() {
  ...
  const formatOutput = (text: string) => {
    ...
    const content = lines.map((line, index) => {
      if (line.match(/^(Q\d+:|Interviewer:)/)) {
        return <p key={index} className="font-semibold text-fuchsia-300 mt-4">{line}</p>;
      }
      if (line.match(/^(A\d+:|.+\:)/) && !line.match(/^(Q\d+:|Interviewer:)/)) {
        return <p key={index} className="text-gray-300 ml-1">{line}</p>;
      }
       if (line.startsWith('[')) {
        return <p key={index} className="text-gray-500 italic text-sm my-2">{line}</p>;
      }
      return <p key={index} className="mt-2 text-gray-300">{line}</p>;
    });
    
    return { title: title || "Generated Interview", content };
  };
  ...
  const { title, content } = formatOutput(output);
            ...
            <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-5 mt-8 lg:mt-0">
                 <div ref={outputRef} className="bg-gray-900/50 backdrop-blur-sm p-6 rounded-2xl border border-purple-900/50 h-[600px] lg:h-[calc(100vh-12rem)] flex flex-col">
                    ...
                    {output && <>
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="whitespace-pre-wrap leading-relaxed space-y-2 overflow-y-auto pr-2">
                            {content}
                        </motion.div>
                    </>}
